...
def normalize_mono(x: np.ndarray) -> np.ndarray:
    """Convierte a mono (promedio) y normaliza a [-1,1] si hay clipping."""
    if x.ndim == 2:
        x = x.mean(axis=1)
    x = x.astype(np.float32, copy=False)
    m = np.max(np.abs(x)) if x.size else 0.0
    # normaliza sólo si excede 1.0; evita amplificar ruidos muy bajos
...
def record_audio(duration_sec: float, fs: int = 48000, channels: int = 1,
    ...
    """Graba 'duration_sec' segundos y devuelve float32 mono normalizado."""
...
def welch_db(x: np.ndarray, fs: int, nperseg: int = 4096):
    f, Pxx = signal.welch(x, fs=fs, window="hann", nperseg=nperseg, noverlap=nperseg//2)
...
def apply_highpass(x: np.ndarray, fs: int, cutoff: float = 1000.0) -> np.ndarray:
...
def detect_beeps(x: np.ndarray, fs: int, use_hpf: bool = True, cutoff_hz: float = 1000.0,
    ...
    y = apply_highpass(x, fs, cutoff_hz) if use_hpf else x
